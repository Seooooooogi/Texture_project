...
class PromptProcessorOutput:
    ...
    def get_text_embeddings(
        ...
        if view_dependent_prompting:
            ...
            tgt_text_embeddings = self.tgt_text_embeddings.expand(  # type: ignore
                batch_size, -1, -1
            )

        # IMPORTANT: we return (cond, uncond), which is in different order than other implementations!
        return torch.cat([text_embeddings, uncond_text_embeddings], dim=0), torch.cat([tgt_text_embeddings, uncond_text_embeddings], dim=0)
...
class PromptProcessor(BaseObject):
    ...
    def configure(self) -> None:
        ...
        # self.tgt_prompt = self.prompt + ', ' + self.cfg.tgt_prompt
