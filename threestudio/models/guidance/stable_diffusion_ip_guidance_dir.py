...
class StableDiffusionIPGuidance(BaseObject):
    ...
    def compute_grad_sds(
        ...
        cond_image_embeddings = torch.cat([cond_image_embeddings_270, cond_image_embeddings_90, cond_image_embeddings_150, cond_image_embeddings_270], dim=0)

        cond_text_embeddings, uncond_text_embeddings = text_embeddings.chunk(2)

        cond_text_embeddings = cond_text_embeddings.repeat(batch_size, 1, 1)
        uncond_text_embeddings = uncond_text_embeddings.repeat(batch_size, 1, 1)
        ...
        with torch.no_grad():
            ...
            down_block_res_samples_1, mid_block_res_sample_1 = self.controlnet(
                ...
                encoder_hidden_states=text_embeddings,
                controlnet_cond=torch.cat([normal]*2*batch_size, dim=0),
                conditioning_scale=1.0,
                guess_mode=True,
                return_dict=False,
            )
            down_block_res_samples_2, mid_block_res_sample_2 = self.controlnet_2(
                ...
                torch.cat([t] * 2 * batch_size),
                encoder_hidden_states=text_embeddings,
                controlnet_cond=torch.cat([canny]*2*batch_size, dim=0),
                conditioning_scale=1.0,
                guess_mode=True,
                return_dict=False,
            )
        ...
        noise_pred_0, noise_pred_90, noise_pred_180, noise_pred_270 = noise_pred.chunk(4)
        noise_pred_uncond, _, _, _ = noise_pred_uncond.chunk(4)
        
        azimuth = azimuth.to(self.device)
        if azimuth < 90 and azimuth >= 0:
            noise_pred_dir = (azimuth / 90) * noise_pred_0 + (1 - azimuth / 90) * noise_pred_90
        elif azimuth < 180 and azimuth >= 90:
            noise_pred_dir = ((azimuth - 90) / 90) * noise_pred_180 + (1 - ((azimuth - 90) / 90)) * noise_pred_90
        elif azimuth < -90 and azimuth >= -180:
            noise_pred_dir = ((abs(azimuth) - 90) / 90) * noise_pred_180 + (1 - ((abs(azimuth) - 90) / 90)) * noise_pred_270
        else:
            noise_pred_dir = (abs(azimuth) / 90) * noise_pred_270 + (1 - abs(azimuth) / 90) * noise_pred_0
        ...
        grad = w * (noise_pred_dir - real_noise[0])
