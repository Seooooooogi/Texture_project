...
@threestudio.register("stable-diffusion-custom-guidance")
class StableDiffusionIPGuidance(BaseObject):
    ...
    def configure(self) -> None:
        ...
        controlnet_model_path = "lllyasviel/sd-controlnet-normal"
        ...
        for i in range(inv_steps):
            ...
            if cfg > 1.0:
                ...
                down_block_res_samples, mid_block_res_sample = self.controlnet(
                    ...
                    timestep_model_input,
                    encoder_hidden_states=text_embeddings,
                    class_labels=torch.zeros_like(c2w.view(cur_noisy_lat_.shape[0], -1)),
                ...
                unet_output = self.unet(
                    ...
                    #     [
                    #         c2w.view(cur_noisy_lat_.shape[0], -1),
                    #     ],
                    #     dim=0,
                    # ),
    ...
    def compute_grad_sds(
        ...
        if self.cfg.scale == -1.0:
            scale = min(int(cosine_schedule(980, 20, t.item()).item()) / 1000, 0.8)
            # print(t.item(), scale)
        ...
        with torch.no_grad():
            ...
            _, prev_latents_noisy, pred_scores_xs = self.add_noise_with_cfg(latents, normal, noise, ind_prev_t, starting_ind, c2w, 1.0, uncond_embeddings, self.cfg.xs_delta_t, self.cfg.xs_inv_steps, eta=0.0)
            _, latents_noisy, pred_scores_xt = self.add_noise_with_cfg(prev_latents_noisy, normal, noise, t, ind_prev_t, c2w, 1.0, uncond_embeddings, current_delta_t, 1, eta=0.0, is_noisy_latent=True)
        ...
        with torch.no_grad():
            ...
            down_block_res_samples, mid_block_res_sample = self.controlnet(
                ...
                class_labels=torch.cat(
                    [
                        c2w.view(batch_size, -1),
                        torch.zeros_like(c2w.view(batch_size, -1)),
                    ],
                    dim=0,
                ),
