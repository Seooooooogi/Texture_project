...
# +
@threestudio.register("styletex-guidance")
class StableDiffusionIPGuidance(BaseObject):
    ...
    def configure(self) -> None:
        ...
        self.content_embeddings = self.pipe.text_encoder(tokens.input_ids.to(self.device))[0]
...
#         negative_image = Image.effect_mandelbrot((224, 224), (-3, -2.5, 2, 2.5), 100)
        ...
        text_style_embeddings = torch.cat([cond_text_embeddings, uncond_image_embeddings], dim=1)
        text_content_embeddings = torch.cat([self.content_embeddings, uncond_image_embeddings], dim=1)
        ...
        with torch.no_grad():
            _, prev_latents_noisy, pred_scores_xs = self.add_noise_with_cfg(latents, normal, canny, noise, ind_prev_t, starting_ind, condition_scale,
                                                                        1.0, uncond_embeddings, self.cfg.xs_delta_t, self.cfg.xs_inv_steps, eta=0.0)
            _, latents_noisy, pred_scores_xt = self.add_noise_with_cfg(prev_latents_noisy, normal, canny, noise, t, ind_prev_t, condition_scale,
        ...
        text_embeddings = torch.cat([cond_embeddings, text_style_embeddings, text_content_embeddings, uncond_embeddings], dim=0)
        ...
        with torch.no_grad():
            ...
            down_block_res_samples, mid_block_res_sample = self.controlnet(
                latent_model_input,
                torch.cat([t] * 4 * batch_size),
                encoder_hidden_states=text_embeddings,
                controlnet_cond=torch.cat([normal]*4, dim=0),
                conditioning_scale=0.5,
                guess_mode=True,
                return_dict=False,
            )
        ...
        # perform guidance
        noise_pred_text, noise_pred_style, noise_pred_content, noise_pred_uncond = noise_pred.chunk(4)
        
        noise_pred = noise_pred_uncond + self.cfg.guidance_scale * (
            noise_pred_style - noise_pred_content
        )

        delta_style = self.cfg.guidance_scale * (noise_pred_text - noise_pred_uncond)
        ...
        noise_pred = noise_pred + delta_style
