...
@threestudio.register("stable-diffusion-ip-consist-guidance")
class StableDiffusionIPConsistGuidance(BaseObject):
    @dataclass
    class Config(BaseObject.Config):
        ...
        perturb_indices: Optional[str] = "sqrt"
    ...
    def compute_grad_sds(
        ...
        self.text_embeddings = torch.cat([cond_embeddings, uncond_embeddings], dim=0)
        ...
        with torch.no_grad():
            ...
            down_block_res_samples_1, mid_block_res_sample_1 = self.controlnet(
                ...
                encoder_hidden_states=text_embeddings,
                controlnet_cond=torch.cat([normal]*2, dim=0),
                conditioning_scale=1.0,
                guess_mode=True,
                return_dict=False,
            )
        ...
        **kwargs,
        ...
        if "consistency" in self.cfg.loss_type:
            grad, loss_sds, grad_weight, sds_weight, guidance_eval_utils = self.compute_grad_consistency(
                latents, t, t_next, prompt_utils, elevation, azimuth, camera_distances, normal_BCHW, canny_BCHW
            )
        else:
            if self.cfg.use_sjc:
