...
from diffusers import AutoencoderKL, DDIMScheduler, DDPMScheduler, ControlNetModel, StableDiffusionControlNetPipeline
...
import sys
import os
sys.path.append('./adapter_instruct')
sys.path.append('./')
...
from threestudio.models.guidance.adapter_instruct.ip_adapter_instruct import *
        
@threestudio.register("stable-diffusion-ip-instruct-guidance")
class StableDiffusionIPInstructGuidance(BaseObject):
    @dataclass
    class Config(BaseObject.Config):
        ...
        instruct_guidance_scale: float = 1.0
    ...
    def configure(self) -> None:
        ...
        #     self.num_tokens=16
        #     ip_ckpt = "threestudio/models/guidance/adapter/models/ip-adapter-plus_sd15.bin"
        # else:
        #     self.num_tokens=4
        #     ip_ckpt = "threestudio/models/guidance/adapter/models/ip-adapter_sd15.bin"
        
        ip_ckpt = "threestudio/models/guidance/adapter_instruct/models/ip-adapter-instruct-sd15.bin"
        self.adapter = IPAdapterInstruct(self.pipe, self.cfg.image_encoder_path, ip_ckpt, device=self.device, dtypein=torch.float16, num_tokens=16)
    ...
    def compute_grad_sds(
        ...
        cond_text_embeddings, uncond_text_embeddings, _, _ = prompt_utils.get_text_embeddings(
            elevation, azimuth, camera_distances, self.cfg.view_dependent_prompting
        )

        # print(cond_text_embeddings.shape)
        
        prompt_ids_instruction = self.tokenizer(
            self.cfg.query, max_length=self.tokenizer.model_max_length, padding="max_length", truncation=True, return_tensors="pt"
        ).input_ids.to(self.device)
        instruct_embeds = self.text_encoder(prompt_ids_instruction, return_dict=True)[0]
        prompt_ids_instruction_negative = self.tokenizer(
            "", max_length=self.tokenizer.model_max_length, padding="max_length", truncation=True, return_tensors="pt"
        ).input_ids.to(self.device)
        negative_instruct_embeds = self.text_encoder(prompt_ids_instruction_negative, return_dict=True)[0]  
        prompt_ids_instruction_everything_temp = self.tokenizer(
            "everything", max_length=self.tokenizer.model_max_length, padding="max_length", truncation=True, return_tensors="pt"
        ).input_ids.to(self.device)
        instruct_embeds_everything = self.text_encoder(prompt_ids_instruction_everything_temp, return_dict=True)[0]

        image_prompt_embeddings_list = torch.empty(0, 16, 768).to(self.device)
        uncond_image_prompt_embeddings_list = torch.empty(0, 16, 768).to(self.device)
        img_only_prompt_embeddings_list = torch.empty(0, 16, 768).to(self.device)
        img_prompt_everything_cond_list = torch.empty(0, 16, 768).to(self.device)
        for cond_t, uncond_t in zip(cond_text_embeddings, uncond_text_embeddings):
            cond_t = cond_t.unsqueeze(0)
            uncond_t = uncond_t.unsqueeze(0)
            image_prompt_embeddings, uncond_image_prompt_embeddings, img_only_prompt_embeddings, img_prompt_everything_cond = self.adapter.get_image_embeds(
                pil_image=self.image, 
                clip_image_embeds=None, 
                instruct_embeds=instruct_embeds, 
                negative_instruct_embeds=negative_instruct_embeds, 
                prompt_embeds=cond_t,
                negative_prompt_embeds=uncond_t,
                instruct_embeds_everything=instruct_embeds_everything
            )
            image_prompt_embeddings_list = torch.cat([image_prompt_embeddings_list, image_prompt_embeddings])
            uncond_image_prompt_embeddings_list = torch.cat([uncond_image_prompt_embeddings_list, uncond_image_prompt_embeddings])
            img_only_prompt_embeddings_list = torch.cat([img_only_prompt_embeddings_list, img_only_prompt_embeddings])
            img_prompt_everything_cond_list = torch.cat([img_prompt_everything_cond_list, img_prompt_everything_cond])
            

        cond_embeddings = torch.cat([cond_text_embeddings, image_prompt_embeddings_list], dim=1)
        uncond_embeddings = torch.cat([uncond_text_embeddings, uncond_image_prompt_embeddings_list], dim=1)
        img_only_prompt_embeddings = torch.cat([uncond_text_embeddings, img_only_prompt_embeddings_list], dim=1)
        img_prompt_everything_cond = torch.cat([uncond_text_embeddings, img_prompt_everything_cond_list], dim=1)
        text_embeddings = torch.cat([cond_embeddings, img_only_prompt_embeddings, img_prompt_everything_cond, uncond_embeddings], dim=0)
        ...
        with torch.no_grad():
            ...
            _, latents_noisy, pred_scores_xt = self.add_noise_with_cfg(prev_latents_noisy, noise, t, ind_prev_t,
        ...
        if self.cfg.scale == -1.0:
            scale = int(cosine_schedule(980, 20, t.item()).item()) / 1000
        else:
        ...
        with torch.no_grad():
            ...
            noise_pred = self.forward_unet(
                latent_model_input,
                torch.cat(([t] * 4 * batch_size), dim=0),
                encoder_hidden_states=text_embeddings,
            )
        ...
        noise_pred = (
            ...
            + self.cfg.instruct_guidance_scale * (noise_pred_full - img_prompt_everything_cond) #instruct
            + self.cfg.guidance_scale * (img_prompt_everything_cond - noise_pred_image_only) #prompt
            + self.cfg.image_guidance_scale * (noise_pred_image_only - noise_pred_fully_uncond) # image 
        )
