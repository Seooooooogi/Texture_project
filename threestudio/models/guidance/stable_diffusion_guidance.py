...
from diffusers import DDIMScheduler, DDPMScheduler, ControlNetModel, StableDiffusionXLPipeline, UNet2DConditionModel
...
class StableDiffusionXLGuidance(BaseObject):
    @dataclass
    ...
    def configure(self) -> None:
        ...
        # text_encoder = CLIPTextModel.from_pretrained(self.cfg.pretrained_model_name_or_path + "/unet", use_safetensors=True)
        # unet = UNet2DConditionModel.from_pretrained(self.cfg.pretrained_model_name_or_path + "text_encoder", use_safetensors=True)
        pipe_kwargs = {
            "tokenizer": None,
            "safety_checker": None,
    ...
    def get_text_embeddings(self, elevation, azimuth):
        
        if azimuth > -45.0 or azimuth < 45.0:
            prompt = self.directions[0]
        elif azimuth < -135.0 or azimuth > 135.0:
            prompt = self.directions[2]
        else:
            prompt = self.directions[1]
    ...
    def compute_grad_sds(
        ...
        cond_text_embeddings, uncond_text_embeddings = prompt_utils.get_text_embeddings(
            elevation, azimuth, camera_distances, self.cfg.view_dependent_prompting
        ).chunk(2)

        only_direction_embeddings, uncond_direction_embeddings = self.get_text_embeddings(
            elevation, azimuth
        )
        ...
        with torch.no_grad():
            ...
            _, prev_latents_noisy, pred_scores_xs = self.add_noise_with_cfg(latents, noise, ind_prev_t, starting_ind,
                                                                        1.0, uncond_text_embeddings, self.cfg.xs_delta_t, self.cfg.xs_inv_steps, eta=0.0)
            _, latents_noisy, pred_scores_xt = self.add_noise_with_cfg(prev_latents_noisy, noise, t, ind_prev_t,
                                                                    1.0, uncond_text_embeddings, current_delta_t, 1, eta=0.0, is_noisy_latent=True)
        ...
        text_embeddings = torch.cat([cond_text_embeddings, uncond_text_embeddings], dim=0)
